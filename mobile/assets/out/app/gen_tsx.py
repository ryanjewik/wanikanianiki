    ...
    while i < len(s):
        ...
        while k < len(s):
            if s.startswith('<g', k):
                ...
                k += 1
        chunk = s[j:k]
        m = re.match(r'<g id="([a-z0-9]+)"', chunk)
...
head = parts['head']
m = re.search(r'<G id="eyes">(.*?)</G>', head, re.S)
parts['eyes'] = m.group(1)
parts['head'] = head.replace(m.group(0), '{eyes}')
json.dump({'order': order, 'parts': parts, 'viewBox': VB, 'clawTF': C.CLAW_TF,

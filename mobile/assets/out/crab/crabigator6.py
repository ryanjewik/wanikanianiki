...
"""
import math
from ink import wob, wobo, pts_of

BK = "#1A1A1A"
# "faceted rock" treatment: punchier palette, a per-part tint shift so each
# traced facet reads as its own plane, and a fine speckle pattern over the top.
# palette "coral reef": red shell, teal claws / boulder, lime head
RED, RED_D, RED_L = "#F5372B", "#C0281C", "#7FD6C4"
GRN = "#9BDC33"
...
# --- facet tint ------------------------------------------------------------
# Each part drawn by block() gets its own small lightness shift, applied in a
# fixed order so a given part always lands on the same tint in every frame.
JITTER = [0.0, .045, -.035, .02, -.055, .06, -.02, .035, -.045, .015,
...
# --- zonal halftone shading -------------------------------------------------
# The reference etching does not wash texture over a whole shape: a terminator
# runs across each form and only the far side is worked, with a second denser
# pass in the core. So for every part I project its vertices onto the light
# direction, place two terminators, and clip the dot screen to the shadow side.
# Clip + paint server only — geometry is untouched, so the rig still works.
LIGHT = (-0.60, -0.80)              # light from the upper left
SHADE = [(.34, 1.9), (.62, 3.1)]    # (terminator fraction, dot radius)
...
def _halftone(rad):
    ...
    if not any(f'id="{did}"' in d for d in _DEFS):
        g = DOT_GAP
        _DEFS.append(
            f'<pattern id="{did}" width="{g}" height="{g}" '
            f'patternUnits="userSpaceOnUse">'
            f'<circle cx="{g*.25:.1f}" cy="{g*.25:.1f}" r="{rad}" fill="{BK}" '
            f'opacity="{DOT_OP}"/>'
            f'<circle cx="{g*.75:.1f}" cy="{g*.75:.1f}" r="{rad}" fill="{BK}" '
            f'opacity="{DOT_OP}"/></pattern>')
    return did
...
def block(paths, fill, amp=None):
    fill = shift(fill, JITTER[_N[0] % len(JITTER)] * 0.7)
    _N[0] += 1
    ...
    o += [_shade(d, raw) for d, raw in zip(w, paths)]
    return "".join(o)
...
BODY = P([(151, 493), (150, 544), (273, 641), (289, 686), (355, 742),
          (345, 808), (361, 819), (394, 816), (431, 762), (465, 752),
          (537, 760), (564, 803),
          (687, 809), (740, 800), (791, 793), (786, 748), (758, 730),
          (742, 668), (740, 626), (784, 584), (752, 522), (720, 452),
          (650, 420),
...
# the opposite arm, for the poses that need both arms doing the same thing:
# the pincer mirrored to the left (both claws up) and the resting fist
# mirrored to the right (both arms down)
CLAW_L_TF = "translate(964 0) scale(-1 1) translate(0 -34)"
ROCK_R_TF = "translate(966 0) scale(-1 1) translate(0 -4)"
...
# ===================================================== reaction fx
# Two overlay layers used only by the answer-feedback poses. Both are ordinary
# outlined shapes in named groups, so they animate through the same rig.
def _spike(ang, r0, r1, half, cx=500, cy=560):
    a = math.radians(ang)
    ax, ay = math.cos(a), math.sin(a)
    px, py = -ay, ax
    return P([(cx + ax * r1, cy + ay * r1),
              (cx + ax * r0 + px * half, cy + ay * r0 + py * half),
              (cx + ax * r0 - px * half, cy + ay * r0 - py * half)])


# rays sit OUTSIDE the silhouette and are drawn behind it, so the creature
# never gets covered by its own celebration
BURST = [_spike(a, 415, 415 + (108 if i % 2 == 0 else 62),
                30 if i % 2 == 0 else 19)
         for i, a in enumerate(list(range(-170, 11, 20)) + [170])]
CHIP = [(112, 250, 22), (900, 214, 18), (330, 116, 15), (700, 108, 13)]

PUFF = [P([(196, 848), (168, 796), (110, 780), (62, 806), (52, 856),
           (86, 894), (152, 900), (190, 884)]),
        P([(884, 840), (860, 788), (802, 774), (754, 802), (748, 852),
           (784, 890), (846, 894), (878, 876)]),
        P([(662, 900), (628, 872), (582, 884), (574, 918), (606, 940),
           (656, 932)])]
PUFF_ARC = [PL([(214, 748), (262, 720), (314, 734)]),
            PL([(724, 742), (774, 716), (824, 730)])]

# ---- eye variants ---------------------------------------------------------
# smooth arcs, not polylines — a jittered three-point V reads as a scribble
EYE_HAPPY = ["M 240 288 Q 274 210 310 284", "M 378 300 Q 415 220 452 292"]
EYE_FLAT = ["M 242 264 L 306 260", "M 378 274 L 450 270"]
...
def _face_only(eye=1.0, eyes='open'):
    ec = f'translate(0 {(1 - eye) * 262:.1f}) scale(1 {eye})' if eye != 1.0 else ''
    ec = f' transform="{ec}"' if ec else ''
    op = lambda k: '1' if eyes == k else '0'
    return (f'<g id="eyes"{ec} opacity="{op("open")}">'
            f'<path d="{wob(EYE_L, 2.5, 34)}" fill="{BK}"/>'
            f'<path d="{wob(EYE_R, 2.5, 34)}" fill="{BK}"/></g>'
            f'<g id="eyesHappy" opacity="{op("happy")}">'
            + "".join(stroke(d, 21) for d in EYE_HAPPY) + '</g>'
            f'<g id="eyesFlat" opacity="{op("flat")}">'
            + "".join(stroke(d, 21) for d in EYE_FLAT) + '</g>'
            + edge(NOSE, 12)
            + "".join(f'<ellipse cx="{x}" cy="{y}" rx="{r}" ry="{r}" '
                      f'fill="{BK}"/>' for x, y, r in NOSTRILS))


def build(mono=False, rig=None, eye=1.0, eyes='open', burst=0.0,
          puff=0.0, arms='default'):
    global RIG
    RIG = rig or {}
    _N[0] = 0
    _DEFS.clear()
    ...
    # arm visibility: 'default' one up one down, 'up' both pincers raised,
    # 'down' both fists resting. Every group is always emitted so the rig can
    # cross-fade them; the pose only sets opacity.
    aop = dict(default=(1, 1, 0, 0), up=(1, 0, 1, 0),
               down=(0, 1, 0, 1))[arms]
    o_claw, o_rock, o_clawL, o_rockR = aop
    rock = (G("boulder", block([BOULDER], light) + edge(BOULDER_EDGE, 16))
            + G("plate", block([PLATE], light)))
    b.append(f'<g opacity="{o_rock}">' + rock + '</g>')
    b.append(f'<g transform="{ROCK_R_TF}" opacity="{o_rockR}">'
             + G("boulderR", block([BOULDER], light) + edge(BOULDER_EDGE, 16))
             + G("plateR", block([PLATE], light)) + '</g>')
    # the left pincer goes down BEFORE the head, so the head overlaps it the
    # way the right pincer overlaps the shell
    b.append(f'<g transform="{CLAW_L_TF}" opacity="{o_clawL}">'
             + G("clawL", block([CLAW], light) + edge(CLAW_EDGE, 12)) + '</g>')

    ec = f'translate(0 {(1 - eye) * 262:.1f}) scale(1 {eye})' if eye != 1.0 else ''
    ec = f' transform="{ec}"' if ec else ''
    # all three eye states are always emitted; the pose picks one by opacity,
    # so the animation rig can cross-fade them without swapping geometry
    op = lambda k: '1' if eyes == k else '0'
    face = (f'<g id="eyes"{ec} opacity="{op("open")}">'
            f'<path d="{wob(EYE_L, 2.5, 34)}" fill="{BK}"/>'
            f'<path d="{wob(EYE_R, 2.5, 34)}" fill="{BK}"/></g>'
            f'<g id="eyesHappy" opacity="{op("happy")}">'
            + "".join(edge(d, 20) for d in EYE_HAPPY) + '</g>'
            f'<g id="eyesFlat" opacity="{op("flat")}">'
            + "".join(edge(d, 20) for d in EYE_FLAT) + '</g>'
            + edge(NOSE, 12)
            + "".join(f'<ellipse cx="{x}" cy="{y}" rx="{r}" ry="{r}" '
                      f'fill="{BK}"/>' for x, y, r in NOSTRILS))
    b.append(G("head", block([HEAD], hbase, 4.0) + face))
    b.append(f'<g transform="{CLAW_TF}" opacity="{o_claw}">'
             + G("claw", block([CLAW], light) + edge(CLAW_EDGE, 12)) + '</g>')

    sc = 0.72 + 0.28 * burst
    b.insert(0, f'<g id="fxBurst" opacity="{burst:.3f}" '
                f'transform="translate(500 560) scale({sc:.3f}) '
                f'translate(-500 -560)">'
                + block(BURST, hbase)
                + "".join(block([P([(x, y - r), (x + r, y), (x, y + r),
                                    (x - r, y)])], light)
                          for x, y, r in CHIP)
                + '</g>')
    b.append(f'<g id="fxPuff" opacity="{puff:.3f}" '
             f'transform="translate(0 {(1 - puff) * 14:.1f})">'
